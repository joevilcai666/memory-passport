...
HMS_BASE = "http://127.0.0.1:18080"
MP_KEY = os.environ.get("MP_API_KEY")
HMS_KEY = os.environ.get("HMS_API_KEY")
if not MP_KEY or not HMS_KEY:
    raise RuntimeError("MP_API_KEY and HMS_API_KEY must be set")
AUTH = {"Authorization": f"Bearer {MP_KEY}"}
RESULT_PATH = Path(__file__).with_name("live_api_results.json")
...
def main() -> None:
    with httpx.Client(timeout=30.0, follow_redirects=True) as client:
        ...
        check("OpenAPI exposes all 26 product operations", len(operations) == 26, sorted(operations))
