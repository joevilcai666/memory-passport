...
The public browser client still uses the evaluator API key configured for this
local V0.1 demo. The documentation must call this an evaluator setup and must
not recommend exposing a production tenant key in a browser. A hosted session
or BFF authentication layer is outside this repair and remains required before
