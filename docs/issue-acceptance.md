...
This matrix records the local acceptance evidence for the V0.1 backend,
product-remediation work, and restore reliability issue #36. Host tests use
isolated databases and mocked HMS failures where appropriate; the Compose suite
runs all service-dependent tests against PostgreSQL and the live deterministic
...
- Frontend: 104/104 tests across 21 files; ESLint pass; optimized Next.js build pass.
...
| #36 Restore | Corrupt archive and unexpected `pg_restore` failure exit non-zero without completion | `make test-restore`: corrupt-archive and `pg_restore` fail-closed cases |
| #36 Restore | One exclusive MP/HMS backup window plus privileged atomic replay | `make test-backup`; `make test-restore` |
| #36 Restore | Archive-exact row counts, vector/Alembic/schema/ownership/access/health | Row-count mismatch test and restore completion gate |
| #36 Restore | Memory text, mapping, retrieval, business migration, audit, and HMS data return | `make test-restore-roundtrip` in a unique disposable Compose project |
...
The local evaluator proves the complete Memory Passport orchestration and lifecycle boundary. It does not substitute for production operator session/RBAC authorization, real provider inference, public TLS, or physical hardware validation; those boundaries are listed explicitly in `docs/validation-report-2026-07-22.md`.
