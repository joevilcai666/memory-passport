...
- **Type/interface consistency:** frontend live checks use the seeded bearer key and `NEXT_PUBLIC_MP_API_URL`; backend HTTP checks use the same `/v1` contract; real HMS uses the same MP boundary and is reported separately.
