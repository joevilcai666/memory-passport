...
Windows 上推荐用 WSL2 跑完整的 `make demo`；原生 PowerShell 的 Compose
等价命令见 [`docs/windows.md`](docs/windows.md)。两条路径在客户签核前仍需
真实 Windows 环境验收。
...
> 浏览器只调用同源 `/api/mp`；`MP_API_URL` 与 `MP_API_KEY` 只存在于 Next.js
> 服务端运行时。生产部署还需要操作员登录与 RBAC；在这层边界完成前，网关在
> production 模式默认关闭。
...
| **Quickstart** | ⭐ **4 步集成向导**，显示本地沙盒 API key，可点 "Run test event" 实测写入 |
...
| **/app/devices** + **/bind** | 注册测试设备，再用后端签发的一次性配对码绑定；当前版本不含摄像头扫码 |
...
UI 之外，你的工程团队会用 API 集成。本地沙盒 API key（写在 Quickstart 页上）：
...
**Q：想给多个客户 / 多租户用？**
后端已经按租户 API key 隔离数据；仓库附带的 Luna 配置只是一个本地沙盒租户。
面向多个正式客户时，仍需自行接入服务端控制面、客户身份、密钥轮换和 RBAC；
不要让不同客户共用仓库中的示例 key。
...
UI 会显示后端返回的错误，保留原页面和原状态，不会提示删除成功。请先确认
后端健康和记忆当前状态；例如状态机不允许的转换需要选择合法操作或重置测试数据。
