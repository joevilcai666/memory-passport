...
snapshot() {
  local mp hms mp_vector hms_vector mp_vector_distance hms_vector_distance
  ...
  hms_vector="$(psql_value "$HMS_DB_NAME" \
    "SELECT count(*) FROM pg_extension WHERE extname = 'vector';")"
  mp_vector_distance="$(psql_value "$MP_DB_NAME" \
    "SELECT '[1,2,3]'::vector <-> '[1,2,4]'::vector;")"
  hms_vector_distance="$(psql_value "$HMS_DB_NAME" \
    "SELECT '[1,2,3]'::vector <-> '[1,2,4]'::vector;")"
  printf 'mp=%s;hms=%s;vector=%s,%s;distance=%s,%s' \
    "$mp" "$hms" "$mp_vector" "$hms_vector" \
    "$mp_vector_distance" "$hms_vector_distance"
}
...
# restore.sh owns the full exclusive writer window and only restarts services
# after every database and application health check succeeds.
